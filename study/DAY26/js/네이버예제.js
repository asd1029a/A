...
$(function(){    
    ...
    setInterval(function(){
      //  var height = "-18px";
        ticker('.search_new_box', 500)
        ticker('.news_cast', 500)
    },2000);
//     setInterval(function(){
//         var height2 = "-20px";
//         ticker2(height2)
...
function ticker(selector, playTime){
    ...
    height = '-'+height;
    $(selector+' li').eq(0).animate({'margin-top': height}, playTime,
    //animate 에서 설정한 애니메이션을 다 실행한 후 실행되는 함수
    function(){
        // $(붙일곳).append(붙일애)  $(붙일애).appendTo(붙일곳)
        $(selector+' li').eq(0).detach().appendTo(selector+' ul').removeAttr('style');
    })
}
